...
struct acceptChallengeView: View {
    @ObservedObject var viewModel: pendingChallengeViewModel
    @ObservedObject var challengeViewModel: challengeViewModel
    @State var challenge: ChallengeTicket
        ...
        NavigationStack {
            ZStack{
                ...
                VStack {
                        ...
                        VStack(spacing: 5) {
                            ...
                            if !viewModel.selectedGames.isEmpty {
                                ForEach(viewModel.selectedGames, id: \.idd) { game in
                                    ...
                                    gameRowViewAcceptCHALLENGE(game: game, isDisabled: false, viewModel: viewModel)
                                    //                                    }
                                }.padding(.horizontal)
                            }
                        }.padding(.bottom,20)
                    ...
                    challengeBetsDisplayAccept(uid: Auth.auth().currentUser!.uid, viewModel: viewModel)
                    ...
                    if viewModel.totalPotentialWon > 0 && !viewModel.totalBetArrays.contains(where: { $0.isEmpty }) {
                        NavigationLink(destination: {
                            finalizeAcceptChallengeView(viewModel: viewModel, challengeViewModel: challengeViewModel).background(K.finalColor.backgroundBlue)
                                .onAppear() {
...
struct gameRowViewAcceptCHALLENGE: View {
    let game: Game
    let isDisabled: Bool
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingSheet) {
            BetDetailsViewAcceptCHALLENGE(game: game, ticketFormat: viewModel.challenge.ticketFormat /*WILL FIX*/, betType: $betType, viewModel: viewModel)
                .padding(.horizontal)
...
// the pop up thing
struct BetDetailsViewAcceptCHALLENGE: View {
    let game: Game
                        ...
                        VStack {
                                ...
                                VStack {
                                    ...
                                    .onAppear {
                                        if let firstNumberGreaterThanZero = viewModel.availableBetsArray.first(where: { $0 > 0 }) {
                                            betNumber = firstNumberGreaterThanZero
                                            print("\(betNumber) this is the bet number")
                                        } else {
                                            betNumber = -99
                                        }
                                        checkTeamTaken()
