//
//  challengeTicketView.swift
//  Weekly Winner
//
//  Created by Reid Brown (Test) on 1/9/24.
//
...
struct challengeBetsDisplay: View {
    let uid: String
                ...
                VStack {
                    ForEach(0..<viewModel.totalBetArrays.count, id: \.self) { parlayIndex in
                        if viewModel.ticketFormat.count > 0 && viewModel.totalBetArrays.count == viewModel.ticketFormat.count {
                            SectionTitle(title: parlayTitle(ticketFormat: viewModel.ticketFormat, index: parlayIndex), betArray: viewModel.totalBetArrays[parlayIndex], maxBetsPlaced: viewModel.ticketFormat[parlayIndex], uid: Auth.auth().currentUser?.uid ?? "", viewModel: viewModel)
                        }
                    }
                }
            }.padding(.bottom,15)
